...
def train(args):
    ...
    with open(
            os.path.join(conf.configPath, args.config + '.yaml'),
    ...
    optimizer = torch.optim.Adagrad(
        model.parameters(), weight_decay=args["lambda_param"])
    ...
    for epoch in range(args["num_epochs"]):
        ...
        for prefix in args["prefix"]:
            #break
            data_set = traffic_data(
                args, data_prefix=prefix, mod="seg", topology=[args["seg"][0]])
            ...
            for seg in args["seg"]:
                ...
                for ii, data in tqdm(enumerate(dataloader)):
                    ...
                    if random.random() < args["sample_rate"]:
                        ...
                        seg_flow_loss = criterion(
                            target[:, :, :, 2].sum(dim=2),
                            number_pred.sum(dim=2))
                        #speed_loss = criterion(target[:, :, :, 3], speed_pred)

                        if args["use_mask"]:
                            mask = get_mask(target[:, :, :, 0], args["mask"])
                            acc_loss = criterion(target[:, :, :, 0],
                                                 output_pred, mask)
                            flow_loss = criterion(target[:, :, :, 2],
                                                  number_pred, mask)
                        else:
                            acc_loss = criterion(target[:, :, :, 0],
                                                 output_pred)
                        ...
                        if args["use_simerror"]:
                            sim_error = outputs[:, :-1, :,
                                                2] - target[:, :-1, :, 2]
                            sim_error = sim_error * (
                                target[:, 1:, :, 0] - outputs[:, 1:, :, 0])
                            sim_error = sim_error_criterion(sim_error)
                        else:
                            sim_error = Variable(
                                data.data.new(1).fill_(0).float())
                    ...
                    if i % args["show_every"] == 0:
                        print(
                            "batch{}, train_loss = {:.3f} for topology {}, preifx {}".
                            format(i,
                                   acc_meter.value()[0], str(seg), prefix))
                        log_curve_file.write(
                            "batch{}, train_loss = {:.3f} for topology {}, preifx {}\n".
                            format(i,
                                   acc_meter.value()[0], str(seg), prefix))
                    i += 1

            print("batch{}, train_loss = {:.3f} for topology {}, preifx {}".
                  format(i,
                         acc_meter.value()[0], str(seg), prefix))
            log_curve_file.write(
                "batch{}, train_loss = {:.3f} for topology {}, preifx {}\n".
        ...
        print("epoch{}, train_loss = {:.3f}, time{:.2f}".format(
            epoch,
            acc_meter.value()[0], t - start))
        log_curve_file.write(
            "epoch{}, train_loss = {:.3f}, time{:.2f}\n".format(
                epoch,
                acc_meter.value()[0], t - start))
        ...
        for prefix in args["test_prefix"]:
            ...
            for seg in args['seg']:
                ...
                for ii, data in tqdm(enumerate(dataloader)):
                    ...
                    flow_loss = criterion(target[:, :, :, 2],
                                          outputs[:, :, :, 2])
                    last_frame_acc_loss = criterion(target[:, -1, :, 0],
                                                    outputs[:, -1, :, 0])
                    last_frame_flow_loss = criterion(target[:, -1, :, 2],
                                                     outputs[:, -1, :, 2])
                    ...
                    if i % args["show_every"] == 0:
                        print(
                            "batch{}, acc_loss={:.3f}, flow_loss={:.3f}, last_frame_loss={:.3f}, last_frame_flow_loss={:.3f}".
                            format(i,
                                   acc_meter.value()[0],
                                   flow_loss_meter.value()[0],
                                   last_frame_acc_meter.value()[0],
                                   last_frame_flow_meter.value()[0]))
                        log_curve_file.write(
                            "batch{}, acc_loss={:.3f}, flow_loss={:.3f}, last_frame_loss={:.3f}, last_frame_flow_loss={:.3f}\n".
                            format(i,
                                   acc_meter.value()[0],
        ...
        print(
            "epoch{}, acc_loss={:.3f}, flow_loss={:.3f}, last_frame_acc_loss={:.3f}, last_frame_flow_loss={:.3f}".
            format(epoch,
                   acc_meter.value()[0],
                   flow_loss_meter.value()[0],
                   last_frame_acc_meter.value()[0],
                   last_frame_flow_meter.value()[0]))
        log_curve_file.write(
            "epoch{}, acc_loss={:.3f}, flow_loss={:.3f}, last_frame_loss={:.3f}, last_frame_flow_loss={:.3f}\n".
            format(epoch,
                   acc_meter.value()[0],
                   flow_loss_meter.value()[0],
                   last_frame_acc_meter.value()[0],
    ...
    print(
        "best mes epoch {}, best mes loss {:.3f}, best flow epoch{}, best flow loss{:.3f}".
        format(best_acc_epoch, best_acc, best_flow_epoch, best_flow_loss))
    log_curve_file.write(
        "best mes epoch {}, best mes loss {:.3f}, best flow epoch{}, best flow loss{:.3f}".
        format(best_acc_epoch, best_acc, best_flow_epoch, best_flow_loss))
