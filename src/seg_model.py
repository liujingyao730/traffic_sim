...
class basic_model(nn.Module):
    def __init__(self, args):
        ...
        self.encoder = seg_model({
            "input_size": self.input_size,
            "hidden_size": self.encoder_hidden
        })
        self.decoder = seg_model({
        ...
        self.embedding_layer = nn.Linear(self.encoder_hidden,
                                         self.decoder_input)
    ...
    def forward(self, input_data):
        ...
        encoder_h = Variable(
            input_data.data.new(batch_size, spatial,
                                self.encoder_hidden).fill_(0).float())
        encoder_c = Variable(
            input_data.data.new(batch_size, spatial,
                                self.encoder_hidden).fill_(0).float())
        decoder_h = Variable(
            input_data.data.new(batch_size, spatial,
                                self.decoder_hidden).fill_(0).float())
        decoder_c = Variable(
            input_data.data.new(batch_size, spatial,
                                self.decoder_hidden).fill_(0).float())

        outputs = Variable(
            input_data.data.new(batch_size, temporal - self.t_predict - 1,
                                spatial, input_size).fill_(0).float())

        for time in range(temporal - 1):

            encoder_h_after, encoder_h_before = self.get_spatial_hidden(
                encoder_h)
            decoder_h_after, decoder_h_before = self.get_spatial_hidden(
                decoder_h)

            data = input_data[:, time, :, :].contiguous()

            encoder_h, encoder_c = self.encoder(
                data, encoder_h, encoder_c, encoder_h_after, encoder_h_before)

            code = self.sigma(self.embedding_layer(encoder_h))

            decoder_h, decoder_c = self.decoder(
                code, decoder_h, decoder_h, decoder_h_after, decoder_h_before)

            if time >= self.t_predict:
                output = self.outputLayer(decoder_h)
                next_input = self.caculate_next_input(
                    input_data[:, time, :, :], input_data[:, time + 1, :, :],
                    output)
...
class attn_model(nn.Module):
    ...
    def get_spatial_hidden(self, h):
        ...
        h_after1 = Variable(
            h.data.new(batch_size, spatial, 1, hidden).fill_(0).float())
        h_before1 = Variable(
            h.data.new(batch_size, spatial, 1, hidden).fill_(0).float())
        h_after2 = Variable(
            h.data.new(batch_size, spatial, 1, hidden).fill_(0).float())
        ...
        h_spatial = torch.cat(
            (h_after2, h_after1, h_before1, h_before2), dim=2)
    ...
    def forward(self, input_data):
        ...
        h = Variable(
            input_data.data.new(batch_size, spatial,
                                self.hidden_size).fill_(0).float())
        c = Variable(
            input_data.data.new(batch_size, spatial,
                                self.hidden_size).fill_(0).float())
...
class attn_model_ad(nn.Module):
    def __init__(self, args):
        ...
        self.encoder = cell_model({
            "input_size": self.input_size,
            "hidden_size": self.encoder_hidden,
            "n_head": self.n_head
        })
        self.decoder = cell_model({
            "input_size": self.decoder_input,
            "hidden_size": self.decoder_hidden,
            "n_head": self.n_head
        })
    ...
    def forward(self, input_data):
        ...
        for time in range(temporal - 1):
            ...
            encoder_h, encoder_c = self.encoder(data, encoder_h, encoder_c,
                                                encoder_spatial)

            code = self.sigma(self.embedding_layer(encoder_h))

            decoder_h, decoder_c = self.decoder(code, decoder_h, decoder_h,
                                                decoder_spatial)
...
class two_type_attn_model_1(nn.Module):
    ...
    def get_spatial_hidden(self, h):
        ...
        h_spatial = torch.cat(
            (h_after3, h_after2, h_after1, h_before1, h_before2, h_after3),
            dim=2)
...
class two_type_attn_model(nn.Module):
    ...
    def get_spatial_hidden(self, h):
        ...
        h_spatial = torch.cat(
            (h_after3, h_after2, h_after1, h_before1, h_before2, h_before3),
            dim=2)
